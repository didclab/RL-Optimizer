...
def run_worker(N, critic_net, env, device, gamma, max_episodes):
    history = []
    memory = ReplaySlice(N)
    thresh = -20 if N == 0 else -15
    
    l_net = ActorNet().to(device)
    ...
    for i in range(max_episodes):
        ...
        # Worker wind-up: lets Critic bootstrap if needed
        if time_epsilon > 0.1:
    ...
    # needed to free reference to shared memory
    memory.free()
    torch.save(critic_net.state_dict(), './assign3_critic_impala_cartp_'+str(N)+'.pth')
    return


class ActorCritic_Impala:
...
if __name__ == "__main__":
    ...
    agent3 = ActorCritic_Impala(env3)
    agent3.Critic_target.share_memory()
    agent3.warm_memory()
    worker_context =  mp.spawn(run_worker,
                args=(agent3.Critic_target, gym.make('CartPole-v1'), agent3.device, 
